...
import { installHermesCommand, showHermesStatus } from './services/hermes.js';
import { startHermesMcpServer } from './mcp/hermes-server.js';
...
;
  ...
  const hermes = program
    .command('hermes')
    .description('Hermes Agent bridge and MCP server integration');

  hermes
    .command('status')
    .description('Show Hermes MCP bridge config, tools, and install status')
    .option('--json', 'Output raw JSON')
    .option('--config-path <path>', 'Override Hermes config path')
    .action((opts) => showHermesStatus(opts));

  hermes
    .command('install')
    .description('Install or update the DARKSOL MCP server entry in Hermes config')
    .option('--server-name <name>', 'Hermes MCP server name', 'darksol')
    .option('--config-path <path>', 'Override Hermes config path')
    .option('--dry-run', 'Print the edited config without writing it')
    .option('--json', 'Output raw JSON')
    .action((opts) => installHermesCommand(opts));

  hermes
    .command('mcp')
    .description('Run the DARKSOL stdio MCP server for Hermes Agent')
    .action(() => startHermesMcpServer());
...
function showCommandList() {
  ...
  const commands = [
    ...
    ['hermes', 'Hermes Agent MCP bridge'],
...
function generateBashCompletion() {
  const commands = [
    ...
    'dash', 'portfolio', 'send', 'receive', 'balance', 'swap', 'history', 'hermes',
    'completion',
  ];
...
function generateZshCompletion() {
  const commands = [
    ...
    'lightning', 'ln', 'support', 'send', 'receive', 'balance', 'swap', 'history', 'hermes', 'completion',
  ];
